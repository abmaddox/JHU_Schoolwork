...
public class QuoteGenerator {
			...
			if (!startDate.isAfter(LocalDate.now())) {
	            ...
	            if(!bookingDay.isValidDate()) {
	                ...
	                if(rates.isValidDates() && rates.getCost() != -0.01d) {
	                	...
	                	errorMessage = wrapErrorInHTML(rates.getDetails());
	                }
	            }
			}
		}
	...
	private String wrapErrorInHTML(List<String> errors) {
		StringBuilder em = new StringBuilder("<p>Your quote could not be generated because:</p>");
		for (String s : errors) {
			em.append("<p>").append(s).append("</p>");
		}
		return em.toString();
	}
